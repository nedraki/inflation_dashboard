...
path_to_country = os.path.abspath(
    "./data/country-currency_code/country-code-to-currency-code-mapping.csv"
)
...
class DataReader:
    ...
    def pct_change(self, df, currency_code, date_reference):
        ...
        df_date_reference = df_exchange.loc[
            (df_exchange.date).dt.date == pd.Timestamp(date_reference)
        ]
    ...
    def metrics(self, df, currency_code):
        ...
        metric_exchange_rate = round(
            implicit_exchange["implicit_exchange"].values[-1], 2
        )
